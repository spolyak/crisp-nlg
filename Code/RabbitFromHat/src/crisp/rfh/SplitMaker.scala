  ...
  def makeSplits(targetL:String, targetR:String, domain:Iterable[String], isRelated:(String,String) => Boolean) = {
    ...
    SubsetGenerator.foreachSubset(domain, { subset:Set[String] =>
      if( !subset.contains(targetL) ) {
        ...
        domain.foreach { x => domain.foreach { y => intersect += ((x,y)) }};
        intersect.retain { a => a match { case ((x,y)) => isRelated(x,y) && !subset.contains(x) && !h.contains(y) } };
    ...
    minimize[(Set[String],Set[String])](splits, {(s1,s2) => if( (s1._1 subsetOf s2._1) && (s1._2 subsetOf s2._2) ) { -1 } else { 1 }});
