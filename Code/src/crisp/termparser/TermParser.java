...
public class TermParser implements TermParserConstants {
  ...
  final public Term term() throws ParseException {
    ...
    if (jj_2_2(2)) {
      ...
      jj_consume_token(9);
                {if (true) return new Compound(label.image, subterms);}
    } else if (jj_2_3(2)) {
      label = jj_consume_token(ATOM);
          {if (true) return new Constant(label.image);}
    } else if (jj_2_4(2)) {
      label = jj_consume_token(VARIABLE);
          {if (true) return new Variable(label.image);}
    } else {
      jj_consume_token(-1);
