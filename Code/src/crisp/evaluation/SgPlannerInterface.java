...
import crisp.evaluation.sgplanparser.SgPlanParser;
...
public class SgPlannerInterface implements PlannerInterface {
    
    public static final String SGPLAN_BIN = "/proj/penguin/planners/sgplan-522-mac";
    ...
    public List<Term> runPlanner(Domain domain, Problem problem) throws Exception {        
        ...
        // Run the SGPlanner
        ProcessBuilder sg_pb = new ProcessBuilder(SGPLAN_BIN, "-o "+TEMPDOMAIN_FILE, "-f "+TEMPPROBLEM_FILE);
        Process sgplanner = sg_pb.start();        
        sgplanner.waitFor();
        if (sgplanner.exitValue() != 0) {
            throw new RuntimeException("SGPlan in "+SGPLAN_BIN+" terminated inappropriately.");
        }        
        
        Reader resultReader = new BufferedReader(new InputStreamReader(sgplanner.getInputStream()));
        try{
            SgPlanParser parser = new SgPlanParser(resultReader);
            return parser.plan();
        } catch(Exception e) {
    ...
    public static void main(String[] args) throws Exception{
        ...
        PlannerInterface planner = new SgPlannerInterface();
