...
public class Planner extends Space {
	...
	private VarArray<IntVar> selectedAction;
	private List<Map<String,TupleSetVariable>> predicate; // predicate.get(step).get(predicatesymbol)
	private VarArray<IntVar> actionParameters;
	...
	public Planner(Domain domain, Problem problem, int plansize) {
		...
		for( Predicate p : domain.getPredicates() ) {
			...
			for( int i = 0; i <= plansize; i++ ) {
				...
				for( String var : p.getVariables().getItems() ) {
