...
public class ActionLayer {
	...
	private int maxApplicableAction;
	private ActionInstanceTable table;
	
	private Set<Integer>[] mutexActionInstances;
	
	private void setMutex(int inst1, int inst2) {
		if( mutexActionInstances[inst1] == null ) {
			mutexActionInstances[inst1] = new HashSet<Integer>();
		}
		
		mutexActionInstances[inst1].add(inst2);
		
		if( mutexActionInstances[inst2] == null ) {
			mutexActionInstances[inst2] = new HashSet<Integer>();
		}
		
		mutexActionInstances[inst2].add(inst1);
	}
	...
	public ActionLayer(State previousState, int maxApplicableAction) {
		...
		mutexActionInstances = new Set[maxApplicableAction+1];
	}
	...
	public boolean isMutex(int inst1ix, int inst2ix) {
		Set<Integer> mutexes = mutexActionInstances[inst1ix];
		
		return mutexes != null && mutexes.contains(inst2ix);
	}
