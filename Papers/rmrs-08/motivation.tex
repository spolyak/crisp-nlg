...
Partial semantic information, especially with respect to ambiguity, is
typically represented using an underspecification formalism such as
\mrs\ \cite{copestake:etal:2005} or CLLS \cite{egg:etal:inpress}.  But
most underspecification formalisms are unable to represent semantic
information that is as partial as what we get from a POS tagger: They
assume that the complete predicate-argument structure is known and
there is no lexical ambiguity.  \rmrs\ \cite{copestake:2007a} was
designed to address this problem.  In \rmrs, we can express the
information we get from the POS tagger as follows:
...
position.  This is useful for \todo{what?}.
...
disambiguation, whereas the chunker and POS tagger didn't.  The fact
...
that can be extracted from a wide range of NLP tools in a uniform way.
