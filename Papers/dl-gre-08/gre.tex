...
(see e.g.,~\newcite{blac:moda01}; \newcite{kurt:expr99}), and there are several
...
Notice that we presented both algorithms as first refining $\RE$
according to the propositional symbols, and then by relational
expressions of increasing depth. But propositional symbols can easily
be encoded by means of new relation symbols (e.g., we could represent
that $f_1$ is a flower in Fig.~\ref{fig:dale-haddock} as a reflexive
loop labeled $\mathsf{flower}$). By means of this encoding, any
arbitrary ordering of preference can be used when generating $\RE$,
