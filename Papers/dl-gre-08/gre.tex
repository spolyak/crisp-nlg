...
according to propositional symbols, and then by relational
expressions of increasing depth. But, actually, propositional symbols can be encoded using new relational symbols (e.g., we could represent
that $f_1$ is a flower in Fig.~\ref{fig:dale-haddock} as a relation labeled $\mathsf{flower}$ from $f_1$ to an additional dummy element $d$).
In this way, any
