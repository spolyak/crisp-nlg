...
\While{for some $(\varphi,S) \in \RE$, $|S| > 1$}{
  \For{$(\varphi,S) \in$ \RE, $R \in \rel$}{
    $\RE \leftarrow$ add$_\el$($\exists R.\varphi$, \RE) \;
  }
...
\For{$(\psi,S) \in \RE$ with $|S| > 1$}{
  \If{$\psi \sqcap \varphi$ is not subsumed in $\RE$ {\bf and}
    $|\psi \sqcap \varphi|^\gM \neq \emptyset$ {\bf and}
    $|\psi \sqcap \varphi|^\gM \neq |\psi|^\gM$}{
    add $(\psi \sqcap \varphi,|\psi \sqcap \varphi|^\gM)$ to $\RE$ \;
...
\KwIn{A model $\gM = (\Delta^\gM, |\cdot|^\gM)$}
\KwOut{A set \RE of pairs $(\varphi,|\varphi|^\gM)$ such that
$\{S \mid (\varphi,S) \in \RE\}$ is the set of $\alc$-bisimulation
classes of $\gM$.}
$\RE \leftarrow \{(\top,\Delta^\gM)\}$
...
\While{for some $(\varphi,S) \in \RE, |S|>1$}{
   \For{$(\varphi,S) \in \RE, R \in \rel$}{
      \For{any set $\{(\psi_1,S_1),\ldots,(\psi_n,S_n)\} \in \RE$}{
         split$((\varphi,S),\exists R.\bigsqcup_i \psi_i,\RE)$
         }
   }
   \If{made no changes to \RE}{
      exit
...
\caption{split$((\varphi,S),\psi, \RE)$}
   \If{$S \cap |\psi|^\gM \not = \emptyset$ and
    $S \backslash |\psi|^\gM \not = \emptyset$}{
         add  $(\varphi \sqcap \psi     , S \cap |\psi|^\gM)$ and
               $(\varphi \sqcap \neg \psi, S \backslash |\psi|^\gM)$ to \RE \;
         delete $(\varphi,S)$ from \RE \;
      }
\end{algorithm}
