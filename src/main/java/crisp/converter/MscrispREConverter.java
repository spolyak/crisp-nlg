...
public class MscrispREConverter implements CrispConverter {
    ...
    public static String defaultCostFilePath = "src/main/resources/costs.txt";	// the file that contains the costs
	...
	List<Term> tlPredicate = null;
	List<String> tlPredicateTypes = null;
	List<Term> subterms = null;
	for (Integer i : problem.getComgoalArities()) {
	    tlPredicate = new ArrayList<Term>();
	    tlPredicateTypes = new ArrayList<String>();
	...
	// especially for mscrisp with RE attribute selection:
	// no positive "needtodecide" literals in the goal state
	List<Term> tlAttrIndiv = new ArrayList<Term>();
	List<String> tlAttrIndivTypes = new ArrayList<String>();
	tlAttrIndiv.add(new Variable("?x1"));
	tlAttrIndivTypes.add("attribute");
	tlAttrIndiv.add(new Variable("?x2"));
	tlAttrIndivTypes.add("individual");
    subterms = new ArrayList<Term>();
    subterms.add(new Variable("?x1"));
    subterms.add(new Variable("?x2"));
    
	Formula noNeedtodecide = new Universal(tlAttrIndiv, tlAttrIndivTypes,
		new Literal(new Compound("needtodecide", subterms), false));
	finalStateGoals.add(noNeedtodecide);
...
//		}
		...
		// Add cost
		double cost = 0;
		if (entry.getCostClass().size() != 0) {
		    cost = computeCost(entry.getCostClass().get(0).toString());
		}
